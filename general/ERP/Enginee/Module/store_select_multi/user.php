...
if($_GET['action']=="SEARCH")	{
	...
	$AddSql = $AddSql." and (name like '%$KEYVALUE%')";
}

echo "<table class=\"TableBlock\" width=\"100%\"><tr class=TableHeader>  <td align=center>仓库</td><td align=\"center\"><b>地址</b></td>\r\n</tr>\r\n\r\n";

$sql = "select * from stock ".$AddSql;
$rs = $db->CacheExecute(150,$sql);
$rs_a = $rs->GetArray();

if(sizeof($rs_a)>0){
for($i=0;$i<sizeof($rs_a);$i++)			{
	$address = $rs_a[$i]['address'];
	$name=$rs_a[$i]['name'];
	$rowid=$rs_a[$i]['ROWID'];
	echo "\r\n<tr id='".$rowid."' title=$name onclick=\"javascript:add_user('";
	echo $rowid;
	echo "','";
	echo "".$name."";
	echo "')\" style=\"cursor:pointer\">\r\n  <td class=TableData align=\"left\" >";
	echo "".$name."";
	echo "</td><td class=TableData>$address</td>\r\n</tr>\r\n";
}
}else{
