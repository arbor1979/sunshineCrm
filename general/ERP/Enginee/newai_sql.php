...
function return_sql_line_mysql($fields)	{
	...
	foreach($temp_array as $list)	{
		...
		switch($filterArray[0])		{
			...
			case 'autoincrement':
				...
				break;
			case 'zhujima':
				$linkfield=$_POST[$filterArray[1]];
				if($linkfield!='')
				{
					
					$temp_post="'".汉字转拼音首字母($linkfield)."'";
				}
				break;	
			case 'input':
	...
	foreach($temp_array as $list)					{
		...
		switch($MODEL_ARRAY[0])		{
			...
			case 'binaryfile':
				...
				break;
			case 'zhujima':
				$linkfield=$_POST[$MODEL_ARRAY[1]];
				if($linkfield!='')
				{
					
					$temp_post="$list='".汉字转拼音首字母($linkfield)."'";
					array_push($temp_update,$temp_post);
				}
				break;
			default:
	...
	for($i=0;$i<sizeof($group_filter_array);$i++)		{
		...
		else if($_GET[$index_name]!='')	
		{
			
			if($index_name=="birthday")
			{
			...
			else if($index_name=="producttype")
			{
				$counter_index=$i;
				$subProdlist=getSubprodtypeByParent($_GET[$index_name]);
				if($subProdlist!='')
					$filter_foreign_arrray[$i]="$index_name in (".$subProdlist.")";
				else 
	...
	if(trim($_GET['searchvalue'])!='')
	{
		...
		if($filter_array[0]=='tablefiltercolor' || $filter_array[0]=='tablefilter')
		{
			...
			$uniquekey_sql_search=str_replace($SYSTEM_ADD_SQL,"",$uniquekey_sql_search);
			$uniquekey_sql_search_array=explode(" where ", $uniquekey_sql_search);
		...
		if($filter_array[0]=='number' || $filter_array[0]=='money')
		{
			$uniquekey_sql_search=str_replace($SYSTEM_ADD_SQL,"",$uniquekey_sql_search);
			$uniquekey_sql_search_array=explode(" where ", $uniquekey_sql_search);
			$otherwhereArray=explode(" and ", $uniquekey_sql_search_array[1]);
			...
			if($otherwhere!='')
				$otherwhere=" and ".$otherwhere;
			$uniquekey_sql_search=$uniquekey_sql_search_array[0]." where ".trim($_GET['searchfield'])."=".$_GET['searchvalue']." $otherwhere $SYSTEM_ADD_SQL";
			$uniquekey_sql_search_array=explode(" where ", $uniquekey_sql_num_search);
			$uniquekey_sql_num_search=$uniquekey_sql_search_array[0]." where ".trim($_GET['searchfield'])."=".$_GET['searchvalue']." $otherwhere $SYSTEM_ADD_SQL";
			$uniquekey_sql_search_array=explode(" where ", $uniquekey_sql_sum_search);
			$uniquekey_sql_sum_search=$uniquekey_sql_search_array[0]." where ".trim($_GET['searchfield'])."=".$_GET['searchvalue']." $otherwhere $SYSTEM_ADD_SQL";
			
		}
		if($filter_array[0]=='city')
		{
			global $db;
			...
			if($insql!='')
			{
				...
				if($otherwhere!='')
					$otherwhere=" and ".$otherwhere;
				$uniquekey_sql_search=$uniquekey_sql_search_array[0]." where $insql $otherwhere $SYSTEM_ADD_SQL";
				$uniquekey_sql_search_array=explode(" where ", $uniquekey_sql_num_search);
				$uniquekey_sql_num_search=$uniquekey_sql_search_array[0]." where $insql $otherwhere $SYSTEM_ADD_SQL";
				$uniquekey_sql_search_array=explode(" where ", $uniquekey_sql_sum_search);
				$uniquekey_sql_sum_search=$uniquekey_sql_search_array[0]." where $insql $otherwhere $SYSTEM_ADD_SQL";
