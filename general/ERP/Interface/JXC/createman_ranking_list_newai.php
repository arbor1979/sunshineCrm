...
//print_r($_GET);
$sql="select b.UID,`b`.`USER_NAME` AS `name`,sum(if(round(c.zhekou,2)<0.36,`c`.`jine`,0)) AS `zhekou<3.6`,
...
from `sellplanmain` `a` left join `user` `b` on a.user_id=b.USER_ID inner join sellplanmain_detail c 
on a.billid=c.mainrowid where a.user_id<>'' and a.user_flag>0 ";
if($start_time!='')
...
$head=array("name"=>"制单员","zhekou<3.6"=>"折<0.36","3.6<=zhekou<3.8"=>"0.36<=折<0.38",
"3.8<=zhekou<4"=>"0.38<=折<0.4","4<=zhekou<5"=>"0.4<=折<0.5",
"5<=zhekou<6"=>"0.5<=折<0.6",
"6<=zhekou<7"=>"0.6<=折<0.7","7<=zhekou<8"=>"0.7<=折<0.8","8<=zhekou<9"=>"0.8<=折<0.9",
"9<=zhekou<1"=>"0.9<=折<1","1<=zhekou"=>"1<=折",
"totalmoney"=>"合计");
...
$title="制单员排行";
$sumcol=array("折<0.36"=>"","0.36<=折<0.38"=>"","0.38<=折<0.4"=>"","0.4<=折<0.5"=>"",
"0.5<=折<0.6"=>"","0.6<=折<0.7"=>"","0.7<=折<0.8"=>"","0.8<=折<0.9"=>"","0.9<=折<1"=>"","1<=折"=>"","totalmoney"=>"");
