...
inner join product b on a.prodid=b.productid inner join sellplanmain c on a.mainrowid=c.billid inner join supply d 
on b.supplyid=d.ROWID where c.user_flag>0";
...
$sql.= " group by d.ROWID,d.supplyname";
//echo $sql;
$rs=$db->Execute($sql);
$rs_b = $rs->GetArray();
...
$head=array("name"=>"供应商","phone"=>"电话","selljine"=>"销售金额","backjine"=>"退货金额","percent"=>"退货率","jine"=>"实际金额");
...
foreach($rs_all as $row)
	{
		...
		foreach ($head as $key=>$val)
		{
			...
			switch($val){
				case "供应商":
					print "<a target='_blank' href='chanpinxiaoshouhuizong.php?".base64_encode("start_time=".$start_time."&end_time=".$end_time."&show_type=table&supplyid=".$row['ROWID'])."'>".$row[$key]."</a>";
					break;
