...
function addnewline()
{
	...
	sendRequest("&action=add&oldproductid="+encodeURIComponent(oldproductid)+"&supplyid=<?php echo $supplyid?>");
	  ...
	  { 
	  	        ...
	  	        else
	  	        {
		    		...
		    		else if(res[0]=='add')
		    		{
		    			var oldprodid=res[1];
		    			var supplyid=res[2];
		    			if(confirm('产品库中没有找到，是否新建？'))
		    				OpenNewProductWindow(supplyid);
		    		}
		    		else
		    		{
		        		alert(data);
...
function OpenNewProductWindow(supplyid)
{
  var oldprodid=form1.oldproductid.value;

  URL = "product_newai.php?action=add_default&oldproductid="+(oldprodid)+"&supplyid="+encodeURIComponent(supplyid);
  loc_y=loc_x=200;
  if(is_ie)
  {
...
<TR><TD colspan=9 class=TableHeader height=30>&nbsp;采购单明细录入&nbsp;（<?php echo $customerName?>）</TD></TR>
</table>
<div id="shoppingcart">
</div>
<div id="inputarea">
